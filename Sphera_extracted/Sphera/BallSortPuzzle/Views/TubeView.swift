...
struct TubeView: View {
    ...
    var explodingColor: BallColor? = nil  // Colore delle palline che esplodono (Color Bomb)
    var showCompletionEffect: Bool = false  // Effetto completamento tubo
    let onTap: () -> Void
    ...
    var body: some View {
                        ...
                        if !(isTopBall && isAnimatingSource) {
                            ZStack {
                                BallView(
                                    ...
                                    isExploding: explodingColor == ball.ballColor
                                )
        ...
        .transaction { $0.animation = nil }
        // Effetto completamento tubo (FUORI dal transaction per permettere animazioni)
        .overlay {
            if showCompletionEffect {
                TubeCompletionAnimatedEffect()
            }
        }
...
struct BallView: View {
    ...
    var isExploding: Bool = false  // Animazione bomba colore
    ...
    var body: some View {
        ZStack {
            // FASE 3: Switch tra le forme
            switch ball.shape {
            case .ball:
                BallShapeView(color: ball.ballColor, size: size)
            case .cube:
                CubeShapeView(color: ball.ballColor, size: size)
            case .pyramid:
                PyramidShapeView(color: ball.ballColor, size: size)
            case .star:
                StarShapeView(color: ball.ballColor, size: size)
            case .diamond:
                DiamondShapeView(color: ball.ballColor, size: size)
            }

            // Indicatore pezzo in cima (sottile glow)
            if isTopBall && !isExploding {
                Circle()
            ...
            if showParticles {
                ExplosionParticlesView(color: ball.ballColor.color, size: size)
            }
        }
        ...
        .onChange(of: isExploding) { _, newValue in
            if newValue {
        ...
        ZStack {
            ForEach(0..<8, id: \.self) { i in
                Circle()
                    ...
                    .offset(
                        x: animate ? cos(Double(i) * .pi / 4) * size * 1.2 : 0,
                        y: animate ? sin(Double(i) * .pi / 4) * size * 1.2 : 0
                    )
                    .opacity(animate ? 0 : 1)
...
// MARK: - Tube Completion Animated Effect

struct TubeCompletionAnimatedEffect: View {
        ...
        ZStack {
            ...
            ForEach(0..<6, id: \.self) { i in
                Image(systemName: "star.fill")
                    ...
                    .offset(
                        x: phase >= 1 ? cos(Double(i) * .pi / 3) * 70 : 0,
                        y: phase >= 1 ? sin(Double(i) * .pi / 3) * 70 : 0
                    )
                    .opacity(phase >= 2 ? 0 : 1)
                    .scaleEffect(phase >= 1 ? 1.2 : 0.3)
                    .animation(.easeOut(duration: 0.4).delay(Double(i) * 0.03), value: phase)
            }
            ...
            Image(systemName: "checkmark.circle.fill")
                ...
                .scaleEffect(phase >= 1 ? (phase >= 2 ? 0 : 1.0) : 0)
...
// MARK: - Ball Shape View (Pallina originale)
