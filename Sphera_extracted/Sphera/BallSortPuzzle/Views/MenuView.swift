                ...
                ScrollView {
                    VStack(spacing: 24) {
                        ...
                        SettingsSection(title: L10n.info) {
                            ...
                            // Support Link
                            Link(destination: URL(string: "https://saimonapps.it/#support")!) {
                                HStack {
                            ...
                            // Privacy Policy Link
                            Link(destination: URL(string: "https://saimonapps.it/#privacy")!) {
                                HStack {
...
struct LevelPickerView: View {
    ...
    @State private var testCoins: Int = 100

    // Livelli comuni per test rapido
    private let quickJumpLevels = [1, 5, 10, 15, 20, 25, 30, 40, 50]

    // Monete preset per test rapido
    private let quickCoinValues = [50, 100, 200, 500, 1000]
                ...
                ScrollView {
                    VStack(spacing: 24) {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            // Quick coin buttons
                            HStack(spacing: 8) {
                                ForEach(quickCoinValues, id: \.self) { value in
                                    Button {
                                        settings.coins = value
                                    } label: {
                                        Text("\(value)")
                                            .font(.system(size: 12, weight: .bold))
                                            .foregroundColor(settings.coins == value ? Color(hex: "F59E0B") : .white)
                                            .padding(.horizontal, 12)
                                            .padding(.vertical, 8)
                                            .background(
                                                RoundedRectangle(cornerRadius: 8)
                                                    .fill(settings.coins == value ?
                                                          Color(hex: "F59E0B").opacity(0.2) :
                                                          Color.white.opacity(0.05))
                                            )
                                            .overlay(
                                                RoundedRectangle(cornerRadius: 8)
                                                    .stroke(settings.coins == value ?
                            ...
                            HStack(spacing: 16) {
                                ...
                                Button {
                                    settings.coins = testCoins
                                } label: {
                                    Text(L10n.current == .italian ? "APPLICA" : "APPLY")
                                        .font(.headline)
                                        .foregroundColor(.white)
                                        .padding(.horizontal, 20)
                                        .padding(.vertical, 10)
