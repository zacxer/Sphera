...
// MARK: - Portal Tube Overlay (ORO - più visibile)
...
struct PortalTubeOverlay: View {
    ...
    // Colore oro fisso per massima visibilità
    private let goldColor = Color(hex: "FFD700")
    private let goldLight = Color(hex: "FFEC8B")

    var body: some View {
        ZStack {
            // Glow di sfondo oro MOLTO visibile
            Circle()
                .fill(goldColor.opacity(glowPulse ? 0.5 : 0.3))
                .frame(width: tubeWidth + 30, height: tubeWidth + 30)
                .blur(radius: 20)

            // Anello esterno rotante ORO
            Circle()
                .stroke(
                    AngularGradient(
                        colors: [
                ...
                .frame(width: tubeWidth + 24, height: tubeWidth + 24)
                .rotationEffect(.degrees(rotation))
                .shadow(color: goldColor.opacity(0.8), radius: 8)

            // Cerchio interno oro
            Circle()
                .fill(
                    RadialGradient(
                        colors: [goldLight.opacity(0.4), goldColor.opacity(0.2)],
                        center: .center,
                        startRadius: 0,
                        endRadius: tubeWidth / 2
                    )
                )
                .frame(width: tubeWidth + 10, height: tubeWidth + 10)

            // Icona portale GRANDE e ORO
            Image(systemName: "arrow.triangle.2.circlepath")
                ...
                .shadow(color: goldColor, radius: 4)
                .shadow(color: .black.opacity(0.5), radius: 2, y: 1)

            // Particelle rotanti ORO più grandi
            ForEach(0..<8, id: \.self) { i in
                Circle()
                    ...
                    .shadow(color: goldColor.opacity(0.8), radius: 3)
                    .offset(x: (tubeWidth / 2 + 18) * cos(CGFloat(i) * .pi / 4 + CGFloat(rotation) * .pi / 180))
                    .offset(y: (tubeWidth / 2 + 18) * sin(CGFloat(i) * .pi / 4 + CGFloat(rotation) * .pi / 180))
            }
        }
        .onAppear {
...
// MARK: - Frozen Ball Overlay (per la pallina in cima)
...
#Preview {
    ZStack {
        ...
        HStack(spacing: 30) {
            ...
            // Portal (ORO)
            VStack {
                PortalTubeOverlay(portalColor: .purple, tubeWidth: 58) // Ora è oro!
                RoundedRectangle(cornerRadius: 25)
                    .fill(Color.white.opacity(0.1))
                    .frame(width: 58, height: 215)
            }
