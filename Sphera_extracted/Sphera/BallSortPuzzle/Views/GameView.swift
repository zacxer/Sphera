...
struct GameView: View {
    ...
    var body: some View {
        ...
        .overlay {
            ...
            // Effetti power-up
            if gameViewModel.isTimerFrozen {
                FreezeTimerEffectView()
            }

            if gameViewModel.showShuffleEffect {
                ShuffleEffectView()
                    .transition(.opacity)
            }

            if gameViewModel.showUndoAllEffect {
                UndoAllEffectView()
                    .transition(.opacity)
            }

            if gameViewModel.showMagicWandEffect {
                MagicWandEffectView()
                    .transition(.opacity)
            }

            if gameViewModel.showColorBombEffect, let color = gameViewModel.explodingColor {
                ColorBombEffectView(color: color.color)
                    .transition(.opacity)
            }
                            ...
                            ForEach(0..<columns, id: \.self) { col in
                                ...
                                if index < tubeCount {
                                    TubeView(
                                        ...
                                        showCompletionEffect: gameViewModel.justCompletedTubeIndex == index,
                                        onTap: {
