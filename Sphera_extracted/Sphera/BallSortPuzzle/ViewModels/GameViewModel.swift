...
class GameViewModel: ObservableObject {
    ...
    @Published var showMagicWandNoSpaceAlert: Bool = false  // Avviso: devi liberare un tubo
    @Published var explodingColor: BallColor? = nil  // Colore delle palline che esplodono
    @Published var isExploding: Bool = false  // Animazione esplosione in corso
    @Published var showShuffleEffect: Bool = false  // Effetto vortice shuffle
    @Published var showUndoAllEffect: Bool = false  // Effetto rewind
    @Published var showMagicWandEffect: Bool = false  // Effetto sparkles
    @Published var showColorBombEffect: Bool = false  // Effetto esplosione bomba
    @Published var justCompletedTubeIndex: Int? = nil  // Indice tubo appena completato (per effetto)
    ...
    private func completeMoveAnimation() {
        ...
        // Controlla se il tubo destinazione era già completo PRIMA della mossa
        let wasCompleteBefore = gameState.tubes[toIndex].isComplete

        // Esegui la mossa reale
        let result = gameState.moveBall(from: fromIndex, to: toIndex)

        // Controlla se il tubo destinazione è diventato completo DOPO la mossa
        if !wasCompleteBefore && gameState.tubes[toIndex].isComplete {
            triggerTubeCompletionEffect(at: toIndex)
        }
    ...
    private func triggerTubeCompletionEffect(at index: Int) {
        justCompletedTubeIndex = index
        AudioManager.shared.playTubeCompleteSound()
        playHaptic(.success)

        // Reset dopo l'animazione
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) { [weak self] in
            self?.justCompletedTubeIndex = nil
        }
    }
    ...
    private func findBestMove() -> (from: Int, to: Int)? {
        ...
        for sourceIndex in 0..<tubes.count {
            // IMPORTANTE: Escludi tubi bloccati e frozen che non possono rilasciare palline
            guard !tubes[sourceIndex].isEmpty,
                  !tubes[sourceIndex].isComplete,
                  !tubes[sourceIndex].isTubeLocked,          // Escludi tubi locked
                  tubes[sourceIndex].canRemoveTop(),          // Escludi tubi frozen
            ...
            for destIndex in 0..<tubes.count {
                guard sourceIndex != destIndex,
                      ...
                      !tubes[destIndex].isTubeLocked,         // Non suggerire destinazione locked
        ...
        for sourceIndex in 0..<tubes.count {
            // IMPORTANTE: Escludi tubi bloccati e frozen
            guard !tubes[sourceIndex].isEmpty,
                  !tubes[sourceIndex].isComplete,
                  !tubes[sourceIndex].isTubeLocked,          // Escludi tubi locked
                  tubes[sourceIndex].canRemoveTop(),          // Escludi tubi frozen
    ...
    private func executeShuffle() {
        // Mostra effetto vortice
        showShuffleEffect = true
        playHaptic(.medium)
        AudioManager.shared.playShuffleSound()

        // Dopo un breve delay, esegui lo shuffle
    ...
    func selectColorForBomb(_ color: BallColor) {
        ...
        // Avvia animazione esplosione GRANDE
        explodingColor = color
        showColorBombEffect = true
        playHaptic(.heavy)
        AudioManager.shared.playColorBombSound()

        // Dopo l'animazione, rimuovi le palline
    ...
    private func executeUndoAll() {
        ...
        // Mostra effetto rewind
        showUndoAllEffect = true
        playHaptic(.medium)
        AudioManager.shared.playUndoAllSound()

        // Dopo un breve delay, esegui il reset
    ...
    private func startMagicWandSelection() {
        // PRIMA verifica: c'è spazio per la magia?
        let hasEmptyTube = gameState.tubes.contains { $0.isEmpty && !$0.isTubeLocked }
        let canAddTube = gameState.tubes.count < 10  // Max 10 tubi totali

        // Se non ci sono tubi vuoti E non possiamo aggiungerne → avviso
        if !hasEmptyTube && !canAddTube {
            showMagicWandNoSpaceAlert = true
            playErrorSound()
            return
        }

        // Conta le palline ACCESSIBILI per ogni colore (escludi tubi locked)
        var colorCounts: [BallColor: Int] = [:]
        for tube in gameState.tubes {
            if tube.isComplete || tube.isTubeLocked { continue }
            for ball in tube.balls {
                colorCounts[ball.ballColor, default: 0] += 1
            }
        }

        // Mostra solo i colori con MAX 4 palline accessibili (capacità tubo normale)
        let validColors = colorCounts.filter { $0.value <= 4 && $0.value > 0 }.map { $0.key }
        ...
        availableColorsForMagicWand = validColors.sorted { $0.rawValue < $1.rawValue }
    ...
    func selectColorForMagicWand(_ color: BallColor) {
        ...
        // Prima cerca un tubo vuoto esistente
        ...
        // Se non c'è un tubo vuoto, AGGIUNGI automaticamente il 10° tubo
        if targetIndex == nil && gameState.tubes.count < 10 {
            // Aggiungi tubo vuoto automaticamente (incluso nel costo della bacchetta)
            let newTube = Tube(type: .normal)
            gameState.tubes.append(newTube)
            targetIndex = gameState.tubes.count - 1
        }

        // Se ancora non c'è un target (già 10 tubi tutti occupati) → errore
        ...
        // Mostra effetto magico
        showMagicWandEffect = true
        playHaptic(.heavy)
        AudioManager.shared.playMagicWandSound()

        // Dopo un breve delay, esegui la magia
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) { [self] in
