...
enum PowerUpType: String, CaseIterable, Codable {
    // Ordinati per costo crescente (30, 50, 75, 120, 150)
    case undoAll        // 30 - Torna all'inizio del livello
    case shuffle        // 50 - Rimescola le palline nei tubi
    case freezeTimer    // 75 - Ferma il timer per 30 secondi
    case magicWand      // 120 - Completa automaticamente un tubo
    case colorBomb      // 150 - Rimuove tutte le palline di un colore (PIÙ POTENTE)
    ...
    var cost: Int {
        ...
        case .magicWand: return 120      // Era 150, ora più accessibile
        case .colorBomb: return 150      // Era 100, più potente = più costoso
        }
    }
}
...
extension L10n {
    ...
    static var powerUpMagicWandDesc: String {
        current == .italian ? "Completa un tubo" : "Complete one tube"
    }

    // UI (altri in SettingsManager.swift)
    ...
    static var magicWandNoSpace: String {
        current == .italian ? "Nessun Tubo Libero!" : "No Empty Tube!"
    }
    static var magicWandNoSpaceDesc: String {
        current == .italian ? "Devi prima liberare un tubo per usare la bacchetta" : "You need to free a tube first to use the wand"
    }
}
