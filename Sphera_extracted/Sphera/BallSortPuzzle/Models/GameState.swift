...
struct GameState: Codable {
    ...
    mutating func processSpecialTubes() -> [SpecialTubeEvent] {
        ...
        for i in 0..<tubes.count {
            ...
            case .locked:
                // Il tubo si sblocca quando QUALSIASI altro tubo è stato completato
                if tubes[i].isTubeLocked {
                    let hasCompletedTube = tubes.enumerated().contains { index, tube in
                        index != i && tube.isComplete
                    }
                    if hasCompletedTube {
                        tubes[i].unlock()
                        events.append(.unlocked(tubeIndex: i))
                    }
                }
