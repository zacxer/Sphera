...
struct TubesContainerView: View {
    ...
    private func calculateLayout(columns: Int, rows: Int, hasTallTube: Bool) -> (horizontalSpacing: CGFloat, verticalSpacing: CGFloat, scale: CGFloat) {
        ...
        case 5:
            // Spacing dinamico basato sul numero totale di tubi
            if tubeCount >= 10 {
                baseLayout = (horizontalSpacing: -8, verticalSpacing: -35, scale: 0.72)
            } else if tubeCount >= 9 {
                baseLayout = (horizontalSpacing: -6, verticalSpacing: -35, scale: 0.76)
            } else {
                baseLayout = (horizontalSpacing: -2, verticalSpacing: -35, scale: 0.82)
            }
        default:
            baseLayout = (horizontalSpacing: -10, verticalSpacing: -40, scale: 0.68)
        }

        // Riduci ulteriormente scala se ci sono 2+ righe
        if rows >= 2 {
            let rowScaleFactor: CGFloat
            if hasTallTube {
                rowScaleFactor = tubeCount >= 10 ? 0.68 : 0.72
            } else {
                rowScaleFactor = tubeCount >= 10 ? 0.72 : 0.78
            }
