...
struct TubesContainerView: View {
    ...
    private func calculateLayout(columns: Int, rows: Int, hasTallTube: Bool) -> (horizontalSpacing: CGFloat, verticalSpacing: CGFloat, scale: CGFloat) {
        ...
        // Base layout per colonne - MOLTO PIÙ COMPATTO
        ...
        case 4:
            baseLayout = (horizontalSpacing: -2, verticalSpacing: -30, scale: 0.85)
        case 5:
            // Spacing molto negativo per 5 colonne
            if tubeCount >= 10 {
                baseLayout = (horizontalSpacing: -14, verticalSpacing: -35, scale: 0.62)
            } else if tubeCount >= 9 {
                baseLayout = (horizontalSpacing: -12, verticalSpacing: -35, scale: 0.65)
            } else {
                baseLayout = (horizontalSpacing: -8, verticalSpacing: -35, scale: 0.72)
            }
        default:
            baseLayout = (horizontalSpacing: -16, verticalSpacing: -40, scale: 0.58)
        }
        ...
        if rows >= 2 {
            ...
            if hasTallTube {
                rowScaleFactor = tubeCount >= 10 ? 0.58 : 0.62
            } else {
                rowScaleFactor = tubeCount >= 10 ? 0.62 : 0.68
            }
