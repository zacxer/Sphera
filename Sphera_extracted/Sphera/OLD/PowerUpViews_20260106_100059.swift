...
/// Effetto Freeze Timer - Overlay ghiacciato MOLTO VISIBILE
struct FreezeTimerEffectView: View {
    @State private var snowflakeOffsets: [CGSize] = (0..<25).map { _ in
        CGSize(width: CGFloat.random(in: -200...200), height: -100)
    }
    @State private var borderPulse: Bool = false

    var body: some View {
        ZStack {
            // Overlay blu ghiacciato più intenso
            Color(hex: "06B6D4").opacity(0.25)
                .ignoresSafeArea()

            // Bordi ghiacciati pulsanti (tutti e 4 i lati)
            VStack {
                // Top
                Rectangle()
                    .fill(
                    ...
                    .frame(height: borderPulse ? 120 : 80)
                Spacer()
                // Bottom
                Rectangle()
                    .fill(
                        LinearGradient(
                            colors: [Color.clear, Color.cyan.opacity(0.3), Color.white.opacity(0.6)],
                            startPoint: .top,
                            endPoint: .bottom
                        )
                    )
            ...
            // Fiocchi di neve grandi
            ForEach(0..<25, id: \.self) { i in
                Image(systemName: "snowflake")
                    .font(.system(size: CGFloat.random(in: 20...40)))
                    .foregroundColor(.white.opacity(0.9))
                    .shadow(color: .cyan, radius: 5)
                    .offset(snowflakeOffsets[i])
            }
            ...
            VStack(spacing: 8) {
                Image(systemName: "snowflake")
                    ...
                    .scaleEffect(borderPulse ? 1.1 : 0.9)

                Text("TIMER CONGELATO")
