...
struct TubesContainerView: View {
    ...
    // Calcola spacing e scala in base al numero di colonne, righe e tipo tubi
    private func calculateLayout(columns: Int, rows: Int, hasTallTube: Bool) -> (horizontalSpacing: CGFloat, verticalSpacing: CGFloat, scale: CGFloat) {
        // Base layout per colonne - SCALA AUMENTATA per tubi più grandi
        var baseLayout: (horizontalSpacing: CGFloat, verticalSpacing: CGFloat, scale: CGFloat)

        switch columns {
        case 1...3:
            baseLayout = (horizontalSpacing: 12, verticalSpacing: -25, scale: 1.1)
        case 4:
            baseLayout = (horizontalSpacing: 4, verticalSpacing: -30, scale: 0.95)
        case 5:
            baseLayout = (horizontalSpacing: -2, verticalSpacing: -35, scale: 0.82)
        case 6:
            baseLayout = (horizontalSpacing: -6, verticalSpacing: -40, scale: 0.74)
        default:
            baseLayout = (horizontalSpacing: -8, verticalSpacing: -45, scale: 0.68)
        }

        // Riduci scala se ci sono 2+ righe
        if rows >= 2 {
            let rowScaleFactor: CGFloat = hasTallTube ? 0.78 : 0.85
            baseLayout.scale = min(baseLayout.scale, rowScaleFactor)
            baseLayout.verticalSpacing = hasTallTube ? -45 : -35
        }
