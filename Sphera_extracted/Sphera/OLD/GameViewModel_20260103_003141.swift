...
class GameViewModel: ObservableObject {
    ...
    private func startMoveAnimation(from sourceIndex: Int, to destIndex: Int, ball: Ball) {
        ...
        // Dopo l'animazione, esegui la mossa reale
        ...
        for sourceIndex in 0..<tubes.count {
            ...
            for destIndex in 0..<tubes.count {
                ...
                if tubes[destIndex].canReceiveBall(topBall) {
                    // Verifica che sia una mossa utile (non spostare su stesso colore se tubo sorgente è già ordinato)
                    let sourceBallsCount = tubes[sourceIndex].consecutiveTopBallsCount
                    let destBallsCount = tubes[destIndex].balls.count
