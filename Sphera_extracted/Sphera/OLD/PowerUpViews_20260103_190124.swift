...
struct PowerUpBarView: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            // Riga unica: Monete + AD | Undo, Hint, Tubo | Power-ups
            HStack(spacing: 6) {
                // Monete + bottone AD
                HStack(spacing: 3) {
    ...
    var body: some View {
        Button(action: {
            ...
            ZStack {
                ...
                Circle()
                    .stroke(isEnabled ? color : Color.gray.opacity(0.4), lineWidth: 1.5)
                    .frame(width: 32, height: 32)
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        // Power-ups list (id forces refresh on changes)
                        ForEach(PowerUpType.allCases, id: \.self) { type in
                            PowerUpShopItem(type: type)
                                .id("\(type.rawValue)_\(settings.getPowerUpCount(type))_\(settings.coins)")
                        }
                    }
                    .padding()
                }
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack {
                    ...
                    Text("x\(settings.getPowerUpCount(type))")
                        ...
                        .foregroundColor(settings.getPowerUpCount(type) > 0 ? .green : .white.opacity(0.5))
                }
