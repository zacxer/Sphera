...
struct TubeView: View {
    ...
    // Altezza dinamica basata sul tipo di tubo
    ...
    var body: some View {
                    ...
                    ForEach(Array(tube.balls.enumerated().reversed()), id: \.element.id) { index, ball in
                        ...
                        if !(isTopBall && isAnimatingSource) {
                            ZStack {
                                ...
                                // Overlay ghiaccio sulla pallina in fondo se frozen
                                if isBottomBall && tube.isBottomBallFrozen {
        ...
        .buttonStyle(.plain)
        // Frame fisso per evitare che l'offset sposti il layout (dinamico per tall tubes)
        .frame(width: tubeWidth + 30, height: tubeHeight + 55)
