...
class GameViewModel: ObservableObject {
    ...
    func selectColorForMagicWand(_ color: BallColor) {
        ...
        // Trova un tubo vuoto o con spazio per questo colore
        var targetIndex: Int? = nil

        // Prima cerca un tubo che già ha palline di questo colore
        for (index, tube) in gameState.tubes.enumerated() {
            if tube.isTubeLocked || tube.isComplete { continue }
            if let topBall = tube.topBall, topBall.ballColor == color {
                targetIndex = index
                break
            }
        }
        ...
        guard let targetIdx = targetIndex else {
            playErrorSound()
            // NON scalare - non si può eseguire l'azione
            return
        }

        // ORA scala le monete/power-up (dopo la selezione e verifica)
        guard settings.usePowerUp(.magicWand) else {
            playErrorSound()
            return
        }

        // Sposta tutte le palline del colore scelto verso il tubo target
        let capacity = gameState.tubes[targetIdx].capacity

        for i in 0..<gameState.tubes.count {
            if i == targetIdx || gameState.tubes[i].isTubeLocked { continue }

            // Rimuovi palline del colore target dalla cima
            while gameState.tubes[targetIdx].balls.count < capacity {
                if let topBall = gameState.tubes[i].topBall,
                   topBall.ballColor == color {
                    _ = gameState.tubes[i].removeBall()
                    _ = gameState.tubes[targetIdx].addBall(topBall)
                } else {
                    break
                }
            }

            if gameState.tubes[targetIdx].balls.count >= capacity { break }
        }
