...
class SettingsManager: ObservableObject {
    ...
    @AppStorage("coins") var coins: Int = 100  // Monete iniziali
    ...
    func calculateCoinsEarned(level: Int, stars: Int, timeBonus: Bool) -> Int {
        var earned = level * 5  // Base: 5 monete per livello
        earned += stars * 3     // Bonus stelle: 3 monete per stella

        // Bonus difficoltà
        switch difficulty {
        case .easy: break
        case .medium: earned = Int(Double(earned) * 1.5)
        case .hard: earned *= 2
        }
