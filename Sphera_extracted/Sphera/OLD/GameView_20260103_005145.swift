        ...
        GeometryReader { geometry in
            VStack(spacing: 12) {
                ...
                GameActionsBar(gameViewModel: gameViewModel)
                    .padding(.bottom, geometry.safeAreaInsets.bottom > 0 ? 60 : 70)
            }
            .padding(.horizontal)
        }
        .overlay {
                                    ...
                                    GeometryReader { geo in
                                        Color.clear
                                            .onAppear {
                                                ...
                                                tubePositions[index] = CGPoint(x: frame.midX, y: frame.minY + 30)
                                            }
                                            .onChange(of: geo.frame(in: .named("tubesContainer"))) { _, newFrame in
                                                tubePositions[index] = CGPoint(x: newFrame.midX, y: newFrame.minY + 30)
                                            }
                                    }
                                )
                            } else {
...
struct FlyingBallView: View {
    ...
    var body: some View {
        BallView(ball: ball, size: 52, isTopBall: false)
